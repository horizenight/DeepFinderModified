...
sys.path.append('../')
from deepfinder.pyqt.custom_theme import set_custom_theme, display_message_box

sys.path.append('../display/')
from deepfinder.pyqt.display.gui_display import DisplayWindow
...
qtcreator_file = 'deepfinder/pyqt/annotation/gui_annotation.ui'
...
class AnnotationWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def on_class_remove(self):
        if len(self.label_list) > 0:
            ...
            if Nobj == 0:  # if class is empty, delete right away
                ...
                reply = QtWidgets.QMessageBox.question(self, 'Remove class', message,
    ...
    def add_object(self, label, coord, obj_id):
        ...
        self.table_objects.setItem(Nobjects, 2, QtWidgets.QTableWidgetItem(
            '(' + str(coord[2]) + ',' + str(coord[1]) + ',' + str(coord[0]) + ')'))
    ...
    def on_button_save(self):
        if len(self.objl) == 0:
            ...
            if self.path_objl == None:  # if path not specified by -o option, then open dialog window
                filename = QtWidgets.QFileDialog.getSaveFileName(self, 'Save object list')
                filename = filename[0]
            else:
            ...
            reply = QtWidgets.QMessageBox.question(self, 'Quit?', message,
                                               QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
            if reply == QtWidgets.QMessageBox.Yes:
                sys.exit()
        ...
        if self.winDisp.dwidget.isTomoLoaded == False:
            ...
            if len(self.objl) != 0:
                ...
                reply = QtWidgets.QMessageBox.question(self, 'Remove class', message,
                                                   QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No)
                if reply == QtWidgets.QMessageBox.Yes:
                    path_objl = QtWidgets.QFileDialog.getOpenFileName(self, 'Open file')
            else:
                path_objl = QtWidgets.QFileDialog.getOpenFileName(self, 'Open file')
