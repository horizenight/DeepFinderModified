...
sys.path.append('../')
from deepfinder.pyqt.custom_theme import display_message_box
...
class DisplayOrthoslicesWidget(QWidget):
    ...
    def initialize_display(self):  # /!\ self.dim needs to be declared first.
        # Stuff that needs to be initialized after loading a volume (tomogram or label map)
    ...
    def initialize_orthoslice_linking(self):  # has to be called after setImage
        ...
        self.vb_xy.setXRange(0, self.dim[0])
        self.vb_xy.setYRange(0, self.dim[1])
        self.vb_zx.setXRange(0, self.dim[2])
        self.vb_zx.setYRange(0, self.dim[0])
        self.vb_zy.setXRange(0, self.dim[2])
        self.vb_zy.setYRange(0, self.dim[1])
